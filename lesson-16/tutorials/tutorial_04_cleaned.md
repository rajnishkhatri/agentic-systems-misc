...
Estimated Reading Time: 18 minutes Prerequisites:
...
Previous Tutorial: Tutorial 03: Deterministic Execution Strategies
Next Tutorial: Tutorial 05: AgentArch Benchmark Methodology
Interactive Notebooks:
Notebook 13: Reliability Framework Implementation - Hands-on error isolation demo
Backend Code:
...
Diagrams:
Error Propagation Cascade - Sequence diagram showing cascade failure
Table of Contents
Introduction
Core Concepts
...
Real-world impact: A financial services company deploys an invoice processing workflow without error isolation. Over 30 days:
...
Definition: Error propagation is the process by which an error in one agent (upstream) causes failures in subsequent agents (downstream) that depend on its output. Propagation occurs when downstream agents:

Accept invalid inputs without validation (no guardrails)
Process corrupted data as if valid (silent corruption)
Amplify errors through compounding inaccuracies (multiplicative degradation)
Key characteristics:
...
Why this matters: Without isolation, debugging becomes exponentially complex. You observe "wrong approver" (symptom at Step 5) but root cause is "vendor hallucination" (5 steps upstream). Traditional stack traces don't help—the error is data corruption, not code exceptions.

Concept 2: Error Propagation Index (EPI)
Definition: The Error Propagation Index (EPI) is a metric that quantifies how many downstream errors result from one upstream failure. It measures the amplification effect of cascade failures.
...
Why EPI matters: EPI is a success metric for isolation effectiveness:
...
Definition: An isolation boundary is an architectural decision point where you contain errors to prevent propagation. Isolation boundaries are implemented through:

Validation gates: Check outputs before passing to next agent
Exception handlers: Catch and contain errors without crashing
Result types: Explicit success/failure values instead of exceptions
Critical/optional distinction: Fail-fast for critical, degrade gracefully for optional
Best practice: Use validation gates after critical agents (where failure is unacceptable) for production systems. See "Designing Isolation Boundaries" for detailed placement strategies and implementation patterns.
...
Crash (exception thrown, workflow terminates)
Produce garbage output (invalid input → invalid output)
Hallucinate (LLM "fills in" missing data with invented values)
Example:
...
Agent1 (Extractor): Parse invoice PDF, extract vendor/amount/date
Agent2 (Validator): Check vendor exists in database
Agent3 (Categorizer): Assign expense category based on vendor
Agent4 (Tax Calculator): Calculate tax based on category + jurisdiction
Agent5 (Router): Route to appropriate approver based on amount + category
...
Example EPI calculations by isolation strategy:
...
Problem with exceptions: Traditional Python exception handling doesn't isolate errors well in async agent workflows:
...
Count total errors
Identify root cause errors (first failure in each workflow)
Count cascaded errors (failures caused by upstream errors)
Calculate EPI using formula: EPI = Cascaded / Root Cause
Solution template:
...
Implement safe_agent_call wrapper that returns Result[T, E]
Refactor workflow to use Result types
Add early termination on critical agent failures
Calculate EPI (should be 0.0 with proper isolation)
Solution template:
...
Extract vendor name from invoice PDF
Validate vendor exists in ERP database
Extract invoice amount from PDF
Categorize expense based on vendor type
Lookup purchase order (PO) to match invoice
Calculate sales tax based on jurisdiction
Route to approver based on amount threshold
Tasks:

Classify each agent as CRITICAL or OPTIONAL
Justify your classification based on business impact
Place validation gates (after which agents?)
Estimate expected EPI with your design
Solution template:
...
Error propagation amplifies failures exponentially
...
Early termination is key to preventing propagation
...
Isolation boundaries are architectural decisions
...
Tutorial 01: Agent Reliability Fundamentals - Cascade failure introduction
Tutorial 03: Deterministic Execution Strategies - Schema validation at boundaries
Tutorial 05: AgentArch Benchmark Methodology - EPI measurement in benchmarks
Backend Code:
...
Notebook 13: Reliability Framework Implementation - Complete isolation demo
Navigation:

← Previous: Tutorial 03: Deterministic Execution Strategies
↑ Index: Tutorial Index
→ Next: Tutorial 05: AgentArch Benchmark Methodology
Feedback: Found an issue or have suggestions? Open an issue or contribute improvements!

Last Updated: 2025-11-23 Version: 1.0 Lesson: Lesson 16 - Agent Reliability

Notebook 14: AgentArch Benchmark Reproduction - EPI across 5 orchestration patterns
