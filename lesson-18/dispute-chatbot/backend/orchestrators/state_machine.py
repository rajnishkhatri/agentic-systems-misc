...
class StateMachineOrchestrator(Orchestrator):
    ...
    def resolve_transition(self, current_state: str, context: dict[str, Any]) -> str:
        """Resolve next state transition dynamically.

        Can be overridden by subclasses for complex routing logic.
        Defaults to first valid transition (linear flow).
        ...
        Raises:
            ValueError: If no transitions defined
        """
        ...
        # Ensure we return a plain string, even if transitions used Enums
        # (str(Enum) returns 'Enum.Name', we want 'Enum.value')
        if hasattr(next_state, "value"):
            return str(next_state.value)

        return str(next_state)

    async def load_state(self, task_id: str) -> dict[str, Any] | None:
        """Load workflow state from latest checkpoint.

        Args:
            task_id: Task identifier

        Returns:
            Latest workflow state dictionary or None if not found
        """
        if self.checkpoint_dir is None:
            return None

        # Find all checkpoints for this task
        # Pattern: {task_id}_state_{state_name}.json
        checkpoints = list(self.checkpoint_dir.glob(f"{task_id}_state_*.json"))
        ...
        return await load_checkpoint(latest_checkpoint)

    async def recover_workflow_from_checkpoint(self, task_id: str) -> dict[str, Any] | None:
        ...
        Returns:
            Recovered workflow state or None
        """
    ...
    async def _execute(self, task: dict[str, Any]) -> dict[str, Any]:
        ...
        Workflow:
        1. Initialize workflow state (new or recovered)
        ...
        recovered_state = await self.recover_workflow_from_checkpoint(task_id)

        if recovered_state:
            self.current_state = recovered_state["current_state"]
            workflow_state = recovered_state
            # If we recovered, we might want to skip the current state if it was already completed?
            # Or do we re-execute the last state?
            # The checkpoint is saved AFTER execution. So if we loaded a checkpoint for state X,
            # it means state X completed. We should proceed to the NEXT state.
            # But the checkpoint contains "current_state" as X (the one just finished).

            # Let's check logic in loop:
            # Loop starts: current_state = self.current_state
            # Execute handler for current_state

            # If we loaded state X, it means X was completed.
            # We should probably transition to next state BEFORE entering loop, or logic needs adjustment.

            # Re-reading _save_state_checkpoint call:
            # await self._save_state_checkpoint(task_id, current_state, workflow_state)
            # This is called after handler execution and before transition.

            # So if we have a checkpoint for "CLASSIFY", it means CLASSIFY is done.
            # We should move to the next state.

            # Resolve next state from recovered data
            next_states = self.transitions.get(self.current_state, [])
            if next_states:
                next_state = self.resolve_transition(self.current_state, workflow_state["accumulated_data"])
        ...
        while True:
            ...
            if current_state in self.state_handlers:
                ...
                try:
                    ...
                    workflow_state["current_state"] = current_state
            ...
            # Resolve next state dynamically
            next_state = self.resolve_transition(current_state, workflow_state["accumulated_data"])
