...
OUTPUT_CSV_PATH = Path(__file__).parent / "synthetic_dispute_queries.csv"
...
def generate_dimension_tuples() -> List[DimensionTuple]:
    ...
    prompt = f"""Generate {NUM_TUPLES_TO_GENERATE} diverse combinations of dimension values for a dispute resolution chatbot.
Each combination should represent a different user scenario. Ensure balanced coverage across all dimensions.

Important: Aim for an even distribution across all dimensions.

DisputeReason:
- fraudulent, product_not_received, product_unacceptable, duplicate, subscription_canceled, credit_not_processed, unrecognized, general

PaymentNetwork:
- visa, mastercard, amex, discover

MerchantServiceType:
- merchandise (physical goods), services (digital/consulting/subscription)

CustomerSentiment:
- neutral, frustrated, confused, formal

TransactionContext:
- high_amount, low_amount, recent_transaction, old_transaction
    ...
    Here are some examples of realistic dispute scenarios to guide generation:
    
    1. Fraud (Visa):
    {{
        "DisputeReason": "fraudulent",
        "PaymentNetwork": "visa",
        "MerchantServiceType": "services",
        "CustomerSentiment": "frustrated",
        "TransactionContext": "high_amount",
        "QueryStyleAndDetail": "detailed_request_high_detail"
    }}
    (e.g., "I never signed up for this! My card was in my wallet... This $500 charge is a scam!")

    2. Merchandise Not Received (Visa):
    {{
        "DisputeReason": "product_not_received",
        "PaymentNetwork": "visa",
        "MerchantServiceType": "merchandise",
        "CustomerSentiment": "neutral",
        "TransactionContext": "recent_transaction",
        "QueryStyleAndDetail": "natural_question_moderate_detail"
    }}
    (e.g., "I ordered a laptop on Oct 1st. Tracking says delivered but I have nothing.")

    3. Subscription Canceled (Visa):
    {{
        "DisputeReason": "subscription_canceled",
        "PaymentNetwork": "visa",
        "MerchantServiceType": "services",
        "CustomerSentiment": "frustrated",
        "TransactionContext": "low_amount",
        "QueryStyleAndDetail": "natural_question_moderate_detail"
    }}
    (e.g., "I canceled my gym membership two months ago... stop charging me!")

    4. Duplicate Charge (Visa):
    {{
    ...
    (e.g., "Two charges for same amount. I only pushed button once.")

    5. Unrecognized (Mastercard):
    {{
        "DisputeReason": "unrecognized",
        "PaymentNetwork": "mastercard",
        "MerchantServiceType": "services",
        "CustomerSentiment": "formal",
        "TransactionContext": "high_amount",
        "QueryStyleAndDetail": "natural_question_moderate_detail"
    }}
    (e.g., "My Mastercard shows a transaction I didn't make.")
...
def generate_queries_for_tuple(dimension_tuple: DimensionTuple) -> List[str]:
    ...
    prompt = f"""Generate {NUM_QUERIES_PER_TUPLE} different natural language queries for a dispute resolution chatbot based on these characteristics:
{dimension_tuple.model_dump_json(indent=2)}

The queries should:
1. Sound like real users reporting a transaction issue
2. Naturally incorporate all the dimension values
3. Vary in style and detail level
4. Be realistic and practical
5. Include natural variations in typing style (typos, case, etc.)
