...
if [ -z "$OPENAI_API_KEY" ]; then
    ...
    echo "Set it in your shell or pass it inline: OPENAI_API_KEY=your-key bash deploy_to_cloud_run.sh"
    echo "Your .env file is not present in the Docker image (to avoid developer keys being used)"
    exit 1
fi
